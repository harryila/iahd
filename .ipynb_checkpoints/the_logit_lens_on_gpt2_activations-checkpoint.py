...
# NOTE: Requires GPU (Tesla T4, P100, A10, A100, etc.)
# Smaller GPT-2 models (774M, 355M, 124M) can run on less powerful GPUs.
#
# SETUP (run these commands before running this script):
#   pip install tensorflow==1.15 colorcet matplotlib pandas numpy scipy tqdm
#   git clone https://github.com/openai/gpt-2.git
#   cd gpt-2
#   python download_model.py 1558M
#
# Then run this script from inside the gpt-2 directory.

model_name = "1558M"  # Options: "124M", "355M", "774M", "1558M"

# Load OpenAI's GPT-2 code and set up TensorFlow session
...
# Helper code for TensorFlow computations
...
# Modified GPT-2 model code to store and return intermediate activations
# multiplied by the embedding matrix (for the logit lens technique)
...
# Define TensorFlow op for the computation
...
# Load pretrained weights into TensorFlow session
...
# Helper code for computing and plotting results
...
def p_of_tokens_frame(results, layer_names, tokens, next_token=True):
  if next_token:
    ...
    iter = list(zip(range(len(tokens)), range(len(tokens)),))
  return pd.DataFrame(
      [[p[i, tokens[j]] for i, j in iter] for p in results['probs']],
      index=layer_names
      )
...
# - They smoothly converge toward the final output
# - Activations quickly stop looking like input tokens
#
# layer_step=2 skips every other layer. Use layer_step=1 for all layers.

# GPT-3 abstract example

tokens, results, layer_names = run_example(gpt3_abstract)

# GPT-3 abstract: first 20 tokens

plot_all(results, layer_names, start_showing_at_token=0, end_showing_at_token=20, layer_step=2)

# GPT-3 abstract: some tokens in the middle
...
# Tests what happens when there's an obvious choice (copy from earlier)
# but with low prior probability (" plasma")
...
# Extreme repetition example
# Where in the network does it figure out the text is repeating?

tokens, results, layer_names = run_example(plasma_repetitive)

plot_all(results, layer_names, start_showing_at_token=0, end_showing_at_token=len(tokens)-1, layer_step=2)

# Navy Seals copypasta example
# Shows what happens with memorized text - the model makes reasonable but wrong
# guesses until the very end, when it suddenly becomes very confident and correct
...
# Scratch space for custom experiments
# Uncomment and modify to run your own examples:
#
# custom_text = "Your text here"
# tokens, results, layer_names = run_example(custom_text)
# plot_all(results, layer_names, start_showing_at_token=0, end_showing_at_token=len(tokens)-1, layer_step=2)
