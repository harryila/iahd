...
# Full context lengths up to 130K (past the 128K limit!)
CONTEXT_LENGTHS = [200, 500, 1000, 2000, 4000, 8000, 10000, 15000, 20000, 30000, 50000, 75000, 100000, 128000, 130000]

print("="*60)
print("LLAMA 3.1 FULL SWEEP UP TO 130K TOKENS (UNSHUFFLED)")
print("="*60)
...
for ctx_len in CONTEXT_LENGTHS:
    ...
    for sample in tqdm(samples, desc=f"{ctx_len} tokens"):
        needle = extract_needle_full_section(sample['section_1'], sample['ground_truth_state'], shuffle=False)
...
# Find next filename
i = 0
while os.path.exists(f'llama31_results/llama31_context_sweep_{i}.json'):
    i += 1
output_path = f'llama31_results/llama31_context_sweep_{i}.json'
...
print(f"\nSaved: {output_path}")
print("\n" + "="*60)
print("LLAMA 3.1 130K SWEEP COMPLETE")
print("="*60)

# Print summary table
print("\nSUMMARY:")
print(f"{'Context':<12} {'Accuracy':<12} {'Correct/Total'}")
print("-"*40)
for ctx_len in CONTEXT_LENGTHS:
    r = results_by_length[ctx_len]
    print(f"{ctx_len:<12} {r['accuracy']*100:<12.1f} {r['correct']}/{r['total']}")
